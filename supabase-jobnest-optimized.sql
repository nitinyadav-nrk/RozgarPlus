...
drop table if exists resume_builders cascade;
...
create table resume_builders (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint resume_builders_experiences_check check (jsonb_array_length(experiences) between 1 and 3)
);
...
create trigger resume_builders_set_updated_at
before update on resume_builders
for each row
execute function set_updated_at();
...
create index if not exists idx_resume_builders_user_id on resume_builders(user_id);
create index if not exists idx_resume_builders_industry_type on resume_builders(industry_type);
...
select setval(pg_get_serial_sequence('resume_builders','id'), coalesce(max(id), 1)) from resume_builders;
